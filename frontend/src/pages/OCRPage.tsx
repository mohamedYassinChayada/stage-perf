...
const OCRPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadDocumentContent = async (): Promise<void> => {
      if (documentId) {
        try {
  ...
  const insertPositionedTextIntoEditor = (ocrResult: OCRResult): void => {
    if (editorRef.current && editorRef.current.getEditor()) {
      ...
      if (!currentContent || currentContent.trim() === '' || currentContent.trim() === canonicalEmptyContent) {
        ...
        if (currentNode && !(currentNode as Element).closest?.('.word-page-content')) {
