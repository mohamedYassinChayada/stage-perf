...
interface Section {
  type: 'header' | 'list-item' | 'indented' | 'paragraph';
  lines: OCRLine[];
  indentLevel: number;
 ...
 */
export const extractTextFromFile = async (file: File, useDetailed: boolean = true): Promise<OCRResult> => {
  try {
    // Validate file type
 ...
 */
export const getOcrInfo = async (): Promise<unknown> => {
  try {
 ...
 */
export const formatTextForEditor = (text: string): string => {
 ...
 */
export const formatPositionedTextForEditor = (ocrResult: OCRResult): string => {
 ...
 */
const groupLinesIntoSections = (lines: OCRLine[]): Section[] => {
 ...
 */
const determineContentType = (text: string, indentLevel: number): Section['type'] => {
 ...
 */
const formatSectionAsEditableHTML = (section: Section): string => {
 ...
 */
export const formatOCRForTinyMCE = (ocrResult: OCRResult): FormattedOCRResult => {
  if (!ocrResult) {
 ...
 */
const formatPDFPagesForTinyMCE = (ocrResult: OCRResult): string => {
  ...
  const pageHtmls = pages.map((pageData, index) => {
    ...
    if (pageLines.length > 0) {
      const singlePageResult: OCRResult = {
        success: true,
        lines: pageLines,
